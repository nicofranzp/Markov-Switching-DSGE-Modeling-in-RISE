...
%% Run the code
m = rise('nk_ms_nonlinear.rs');
p = struct('beta',1/(1+0.706/400),'sigma',2.9,'varphi',2.5,'theta',0.82,...
'gamma',0.77,'zeta',0.10,'eta',10,'nu',1,'rho_z',0.90,'rho_mu',0.70,...
'rho_xi',0.80,'rho_r',0.79,'PaiT',1.,'sig_r',0.10/100,...
'sig_z',0.50/100,'sig_mu',0.15/100,'sig_xi',0.10/100,...
...
m = set(m,'parameters',p);isnan(m);
 m0 = solve(m); print_solution(m0); % brute force
%% --- approaches to solution ---
% provide a steady state fiile
ms1 = solve(m,'sstate_file',@nk_nonlinear_ssfile); print_solution(ms1);
resid(set(m,'sstate_imposed',true,'sstate_file',@nk_nonlinear_ssfile));
% brute force + block decomposition
ms2 = solve(m,'sstate_blocks',true); print_solution(ms2);
ms21 = solve(m,'sstate_blocks',true,'debug',true,'sstate_solver',...
{'lsqnonlin','MaxIter',2000,'MaxFunEvals',1000000}); print_solution(ms21);
ms22 = solve(m,'sstate_blocks',true,'debug',true,'sstate_solver',...
{'fsolve','MaxIter',2000,'MaxFunEvals',1000000}); print_solution(ms22);
